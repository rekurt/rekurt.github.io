...
Полная production-проверка семейства без следования редиректам запускается командой `make site-fleet-check`. Она проверяет только Pages URL из каталога: build manifest, canonical, JSON-LD, языковые маршруты, sitemap, безопасные ресурсы, ссылку на авторский hub и ссылки на все соседние проекты.
