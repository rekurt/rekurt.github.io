...
Индивидуальные лендинги описаны в `internal/projectsite/profiles/<slug>.json`: название, демонстрация, тема, целевое действие и полные тексты на английском, русском и упрощённом китайском. Для нового лендинга возьмите существующий профиль за пример и добавьте его визуальное оформление в `assets/marketing.css`. Общий генератор сохраняет навигацию и актуальные данные репозитория; существующие самостоятельные сайты сохраняют свой дизайн через `decorate`.

После публикации запустите `make marketing-check`: проверяются локализованные тексты, целевые действия, темы, изображения и связи с каталогом. Профили редактируются осмысленно, а синхронизация обновляет версии и документацию, не переписывая маркетинговые тексты. Дочерние workflow подхватывают изменения общего генератора при следующем push, релизе, ручном запуске или по шестичасовому расписанию.
